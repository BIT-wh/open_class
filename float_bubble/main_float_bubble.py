...
KNOWLEDGE_MAP = {
    "1": {
        ...
        "content": (
            "【基本概念】\n"
            "雾霾是雾(Fog)和霾(Haze)的合称，雾霾的核心有害成分是 PM2.5(空气动力学直径 ≤ 2.5 微米的颗粒物)，现代城市雾霾核心是人为排放物在大气中发生复杂化学反应生成的复合污染体系。\n\n"
            "【核心构成 · 占比分析】\n\n"
            "📊 二次无机气溶胶 (40%—60%)\n"
            "   这是造成能见度下降和健康危害的关键成分：\n"
            "   ▹ 硝酸盐：10%—30% (主要来自机动车/工业)\n"
            "   ▹ 硫酸盐：10%—25% (主要来自燃煤/重油，主要以硫酸铵硝酸铵的形式存在)\n"
            "   ▹ 铵  盐：5%—15% (主要来自农业氨气)\n\n"
            "🧪 其他主要物质：\n"
    ...
    "3": {
        ...
        "content": (
            ...
            "· 年均投入超过 3000亿元，仅北京一地十年治霾就投入了 7600亿元。\n"
            "· 主要用于燃煤电厂和钢铁行业排放改造、淘汰老旧柴油车、北方清洁取暖（“煤改气”“煤改电”）等关键工程。\n\n"
            "【回报：健康与经济】\n"
            "· 浓度下降：PM2.5年均浓度从72降至29.3 μg/m³。\n"
            "· 经济效益：每投入1元治霾，可产生2—8元的综合回报(包含医疗支出减少、生产率提升)。\n"
            "· 推动绿色转型：新能源汽车保有量全球第一，非化石能源占比持续提高，超低排放技术实现国产化。\n\n"
...
<script>
    ...
    class Bubble {
        ...
        update() {
            ...
            // --- 1. 设定目标半径逻辑 ---
            if (this.id === "7") {
                this.targetR = this.isHover ? 65 : 60;
            } else {
                // 其他气泡随 targetVal (数量) 动态增长
                this.targetR = (65 + (this.targetVal * 8)) * (this.isHover ? 1.15 : 1);
            }
            ...
            if (this.id === "7") {
                ...
                let dynamicR = (65 + (this.currVal * 8)) * (this.isHover ? 1.15 : 1);
